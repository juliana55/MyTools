...
//	UUSBD_ASSERT( UsbdPipeTypeBulk == pipeHandle->PipeType );
    ...
    UUSBD_KdPrint( DBGLVL_DEFAULT,("length =%d\n",length));
...
--*/
{
	...
	if ( Irp->MdlAddress ) { // could be NULL for 0-len request
		if(	totalLength > MmGetMdlByteCount(Irp->MdlAddress)){
	...
	ChunkSize = pipe->PipeInfo.MaximumTransferSize;
...
//	UUSBD_ASSERT( UsbdPipeTypeBulk == pipeHandle->PipeType );
	...
	UUSBD_ASSERT(totalLength > ChunkSize );
...
//	Irp->IoStatus.Status = STATUS_SUCCESS; 
	if(Irp->IoStatus.Status != STATUS_SUCCESS){
	    UUSBD_KdPrint ( DBGLVL_DEFAULT,("complete with error status = 0x%x\n\n",Irp->IoStatus.Status )); 
	}
	if(urb->UrbHeader.Status == 0) {
	    Irp->IoStatus.Information =
	        urb->UrbBulkOrInterruptTransfer.TransferBufferLength;
	} else {
	    UUSBD_KdPrint ( DBGLVL_DEFAULT,("Urb header status not zero status = 0x%x\n\n",urb->UrbHeader.Status )); 
		Irp->IoStatus.Status = STATUS_IO_DEVICE_ERROR; // return urb status
	    Irp->IoStatus.Information = 0;
	}
...
Uusbd_AbortPipes_sub(PDEVICE_OBJECT DeviceObject, PUUSBD_PIPE pipe)
{
	...
	UUSBD_KdPrint( DBGLVL_HIGH,("UUSBD_AbortPipes() Aborting open  Pipe\n"));
