...
{
	return OpenPipe(husb, interface_num, pipe_num, FALSE);
}
HANDLE APIENTRY Uusbd_OpenPipe_Overlapped(HUSB husb, UCHAR interface_num, UCHAR pipe_num)
{
	return OpenPipe(husb, interface_num, pipe_num, TRUE);
}
...
{
	return GetOtherSpeedConfigurationDescriptor(get_handle(husb), buf, siz);
}
...
{
	return GetDeviceQualifierDescriptor(get_handle(husb), descriptor);
}
