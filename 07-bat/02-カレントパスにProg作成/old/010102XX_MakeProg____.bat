...
set RVL=01010203
set _imgRVL=01010203
set FileName=DP
set inpath=%basedir%HENKATOOL\MANAGER\AMS-F_010102.he2
...
for /f "tokens=1* delims==" %%a in ('findstr /n /i %searchFN% %inpath%') do (
 set line=%%a
 set KEY3=%%b
)
...
echo before2   : %before2% >>%LOG%
echo after2    : %after2% >>%LOG%
echo before3   : %before3% >>%LOG%
echo after3    : %after3% >>%LOG%
if exist %outpath% (
 del %outpath%
)
for /f "tokens=1* delims=: eol=" %%a in ('findstr /n "^" %inpath%') do (
 ...
 if not "!line2!" == "" (
  ...
  set line2=!line2:%before2%=%after2%!
  set line2=!line2:%before3%=%after3%!
 )
 echo.!line2!>> %outpath%
)
endlocal
